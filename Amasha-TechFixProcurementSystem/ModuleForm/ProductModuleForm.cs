...
namespace Amasha_TechFixProcurementSystem.ModuleForm
{
    public partial class ProductModuleForm : Form
    {
        ...
        public void LoadSupplier()
        {
            ...
            cm = new SqlCommand("SELECT sname FROM tbSupplier", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                comboSup.Items.Add(dr[0].ToString());
            }
            dr.Close();
            con.Close();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you sure you want to save this product?", "Saving Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO tbProduct(pname,pqty,pprice,pdescription,pcategory,psuppliername)VALUES(@pname, @pqty, @pprice, @pdescription, @pcategory, @psuppliername)", con);
                    ...
                    cm.Parameters.AddWithValue("@psuppliername", comboSup.Text);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Are you sure you want to update this product?", "Update Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE tbProduct SET pname = @pname, pqty=@pqty, pprice=@pprice, pdescription=@pdescription, pcategory=@pcategory, psuppliername=@psuppliername WHERE pid LIKE '" + lblPid.Text + "' ", con);
                    cm.Parameters.AddWithValue("@pname", txtPName.Text);
                    cm.Parameters.AddWithValue("@pqty", Convert.ToInt16(txtPQty.Text));
                    cm.Parameters.AddWithValue("@pprice", Convert.ToInt16(txtPPrice.Text));
                    cm.Parameters.AddWithValue("@pdescription", txtPDes.Text);
