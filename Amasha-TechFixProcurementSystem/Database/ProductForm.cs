...
namespace Amasha_TechFixProcurementSystem.Database
{
    public partial class ProductForm : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE CONCAT(pid, pname, pprice, pdescription, pcategory, psuppliername) LIKE '%" + txtSearch.Text + "%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                productModule.comboSup.Text = dgvProduct.Rows[e.RowIndex].Cells[7].Value.ToString();
