...
namespace Amasha_TechFixProcurementSystem.Logics
{
    public partial class UserForm : Form
    {
        ...
        public void LoadUser()
        {
            int i = 0;
            dgvUser.Rows.Clear();
            cm = new SqlCommand("SELECT * FROM tbUsers", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                UserModuleForm userModule = new UserModuleForm();
                userModule.txtRole.Text = dgvUser.Rows[e.RowIndex].Cells[1].Value.ToString();
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbUsers WHERE username LIKE '" + dgvUser.Rows[e.RowIndex].Cells[2].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
